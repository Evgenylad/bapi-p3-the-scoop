...
function createComment(url, request) {
  ...
  if (createComment && createComment.body && createComment.username && createComment.articleId && database.users[createComment.username] && database.articles[createComment.articleId]) {
...
function updateComment(url, request) {
  ...
  if (!updateComment || !updateComment.body) {
    response.status = 400;
  } else if (!database.comments[id]) {
    response.status = 404;
  } else {
    database.comments[id].body = updateComment.body;
    response.body = {comment: database.comments[id].body};
    response.status = 200;
  }
  return response;
}
...
function deleteComment(url, request) {
  ...
  if(!id || !savedComment) {
    ...
    let commentIds = database.articles[savedComment.articleId].commentIds;
    commentIds.splice(commentIds.indexOf(id), 1);
    let userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
